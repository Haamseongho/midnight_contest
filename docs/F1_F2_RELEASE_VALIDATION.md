...
> 후속 정정: 아래 U3-3 미체크 사유 중 ‘시각 원자료 없음’은 원본 재확인으로 해소했다. `76d6610`/`f87aee3` 문서의 검증 시각은 **2026-09-25 10:46 UTC (19:46 KST)**, 분 단위 기록이며 블록 포함 시각을 새로 검증한 것은 아니다. 이 문서의 CI/Pages는 기존 `3630642` 릴리스 이력이다. 이후 ZIP 후보의 검수·미배포 상태는 [통합 검증](./FINAL_INTEGRATION_VALIDATION.md)을 따른다.
