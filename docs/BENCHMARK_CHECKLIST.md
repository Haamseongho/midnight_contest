...
| --- | --- | --- | --- |
| 국내 위임(Wiim)·성희청과 | 주최자 공지에 위임장 서비스와 DID 기반 위임·공증 플랫폼으로 기재. 결선 진출 사실 확인; 수상·구현 품질은 이 자료만으로 단정 불가. [OpenDID 공지](https://opendid.org/hackathon/2025/) | 업무를 역할별로 연결하는 설계 원칙을 적용: `index.html`의 주최자 → 소지자 → 확인자, `trusted-context.ts`의 맥락 출처 | [x] 역할과 책임이 표시되고 소지자 입력이 검증자 조회로 전달되지 않음. 원문의 ‘We-im’과 공식 ‘Wiim’ 명칭 동일성은 추가 자료 필요. |
| 국내 zkBLOCK·TodayRPG·Typha | 같은 공식 공지에서 각각 구인구직, 조건·출석·신원 확인 모임, 모바일 신분증/NFT 티켓팅이라는 문제 범위를 확인. [OpenDID 공지](https://opendid.org/hackathon/2025/) | 특정 사용자와 필요한 정보를 먼저 설명하는 원칙을 적용: 첫 화면의 초대 소비 문제, 소지자의 비밀 입력과 확인자의 공개 기록 | [x] 문제→행동→보이는 정보 순서 적용. DID·모바일 신분증·암표 방지 기능을 Silent Pass에 구현했다고 주장하지 않음. |
...
- 국내 사례의 상세 소스·실제 배포·최종 수상 내역은 위 결선 공지만으로 확인할 수 없다. 위 역할/문제 중심 적용은 공개 설명에서 도출한 설계 판단이다.
